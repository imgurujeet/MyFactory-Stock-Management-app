...
@Composable
fun PipeEntryScreen(navHost: NavHostController,onBack : () -> Unit,viewModel: PipeViewModel = hiltViewModel()){
    ...
    val entries by viewModel.pipeEntries.collectAsState()
    ...
    val weightValid by remember(pipeSize, gauges, pipeType, numberOfPipes, scrapWeight, cutPieceWeight) {
        derivedStateOf {
            val approx = approxWeight.toDoubleOrNull() ?: return@derivedStateOf false
            val scrap = scrapWeight.toDoubleOrNull() ?: return@derivedStateOf false
            val cut = cutPieceWeight.toDoubleOrNull() ?: return@derivedStateOf false
            scrap < approx && cut < approx && (scrap + cut) <= approx
        }
    }
    ...
    Scaffold (
        ...
        Column(
            ...
            if (!weightValid && scrapWeight.isNotEmpty() && cutPieceWeight.isNotEmpty()) {
                Text(
                    text = "Scrap + CutPiece ≤ Approx Weight",
           ...
           Button(
               onClick = {
                   ...
                   val scrap = scrapWeight.toDoubleOrNull() ?: 0.0
                   val cutPiece = cutPieceWeight.toDoubleOrNull() ?: 0.0
                   ...
                   if (uid != null) {
                       ...
                       db.collection("users").document(uid)
                           ...
                           .addOnSuccessListener { document ->
                               ...
                               val newScrap = ScrapStock(
                                   gauge = gauges,
                                   grade = grades,
                                   weight = scrap.round(2),
//                                   cutPieceWeight = cutPiece.round(2),
                               ...
                               val newCutPiece = CutPieceStock(
                                   gauge = gauges,
                                   grade = grades,
                                   weight = cutPiece.round(2),
//                                   cutPieceWeight = cutPiece.round(2),
                                   //  approxWeight = approx.round(2),
                                   entryUserId = uid,
                                   entryUserName = name
                               )
                               viewModel.addPipeScrapCutPieceStockWithEntry(newPipe,newScrap,newCutPiece)
               ...
               modifier = Modifier.fillMaxWidth(),
               shape = RoundedCornerShape(10.dp),
               enabled = !loading && (scrapWeight.toIntOrNull()?:0) > 0 && (cutPieceWeight.toIntOrNull()?:0) > 0 && (numberOfPipes.toIntOrNull()?:0) > 0 && coilSize.isNotEmpty() && gauges.isNotEmpty() && grades.isNotEmpty() &&
                       weightValid
